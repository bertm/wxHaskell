...
depends_for_ghc(){
	...
	then
        ...
        then
	        ...
	        pkgname=$(echo $pkgid | sed 's/^\([^-]\+\).*/\1/')
	        pkgver=$(echo $pkgid | sed 's/^.*-\([^-]\+\)-.*/\1/')
	        pkghash=$(echo $pkgid | sed 's/^.*-\([a-f0-9]\+\)$/\1/' | head -c 5)
	        dep=libghc-$pkgname-dev-$pkgver-$pkghash
	        echo "WARNING: No Debian package provides haskell package $pkgid. Assuming $dep." >&2
	        packages="$packages, $dep"
        fi
    else
	    packages="$packages, $dep"
